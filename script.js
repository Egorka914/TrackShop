...
function setupMobileSearch() {
  ...
  const mobileSearchSubmit = $('#mobileSearchSubmit');
  const mobileSearchSuggestions = $('#mobileSearchSuggestions');
  ...
  function updateSearchSuggestions(query) {
    ...
    suggestions.forEach(product => {
      ...
      suggestion.className = 'mobile-search-suggestion';
      suggestion.innerHTML = `
        <div class="suggestion-name">${product.name}</div>
        <div class="suggestion-category">${getCategoryName(product.category)} • ${product.price.toLocaleString()} руб.</div>
      `;
      suggestion.addEventListener('click', () => {
        mobileSearchInput.value = product.name;
        performMobileSearch();
      });
      mobileSearchSuggestions.appendChild(suggestion);
    });
  }
  ...
  function performMobileSearch() {
    ...
    // Scroll to catalog and show results
    $('#catalog').scrollIntoView({ behavior: 'smooth' });
    renderProductsFromList(found);
    showNotification(`Найдено ${found.length} товаров`);
  }
}
